...
export function useBudgets(dateFilter?: { start: Date | null; end: Date | null }) {
  ...
  // Stable reference to "now" — won't change on re-renders
  ...
  const fetchBudgets = useCallback(async () => {
    ...
    if (spendingError) {
      toast({ title: 'Error fetching spending data', description: spendingError.message, variant: 'destructive' });
      setIsLoading(false);
      return;
    }
    ...
    const budgetsWithSpending: BudgetWithSpending[] = (budgetsData || []).map(budget => {
      ...
      if (isOverBudget) status = 'over-budget';
      else if (percentUsed >= 80 || projectedSpend > budgetAmount) status = 'at-risk';

      return { ...budget, spent, percentUsed, remaining, isOverBudget, expectedSpend, expectedPercent, status, projectedSpend, projectedOverage };
    });
  ...
  const summary: BudgetSummary = useMemo(() => {
    ...
    const overBudgetAmount = budgets.filter(b => b.isOverBudget).reduce((sum, b) => sum + (b.spent - Number(b.amount)), 0);
    const categoriesOverBudget = budgets.filter(b => b.isOverBudget).length;
    return { totalBudget, totalSpent, netRemaining, overBudgetAmount, categoriesOverBudget, daysRemaining, daysElapsed, daysInMonth: daysInRange, percentMonthElapsed };
  ...
  const insights: BudgetInsight[] = useMemo(() => {
    ...
    budgets.filter(b => b.isOverBudget).forEach(b => {
      ...
      result.push({ id: `over-${b.id}`, type: 'warning', category: b.categories?.name, message: `${b.categories?.name || 'Unknown'} is over budget by $${overage.toFixed(0)}`, action: 'Review transactions' });
    });
    budgets.filter(b => b.status === 'at-risk' && !b.isOverBudget).forEach(b => {
      const projectedOver = b.projectedSpend - Number(b.amount);
      if (projectedOver > 0) {
        result.push({ id: `risk-${b.id}`, type: 'warning', category: b.categories?.name, message: `You're likely to exceed ${b.categories?.name || 'budget'} by $${projectedOver.toFixed(0)} this month`, action: `Reduce daily spending by $${(projectedOver / daysRemaining).toFixed(0)}` });
      }
    });
    const onTrackCount = budgets.filter(b => b.status === 'on-track').length;
    if (onTrackCount > 0 && budgets.length > 0) {
      result.push({ id: 'on-track-summary', type: 'success', message: `${onTrackCount} of ${budgets.length} budgets are on track for this month` });
    }
    budgets.filter(b => b.percentUsed < 50 && percentMonthElapsed > 50).forEach(b => {
      result.push({ id: `saving-${b.id}`, type: 'info', category: b.categories?.name, message: `${b.categories?.name || 'Category'} spending is ${(b.expectedPercent - b.percentUsed).toFixed(0)}% below expected` });
    });
  ...
  const createBudget = useCallback(async (categoryId: string, amount: number, period: string = 'monthly') => {
    ...
    const { error } = await supabase.from('budgets').insert({ user_id: user.id, category_id: categoryId, amount, period });
    if (error) { toast({ title: 'Error creating budget', description: error.message, variant: 'destructive' }); return false; }
    await fetchBudgets();
    return true;
  }, [toast, fetchBudgets]);
  ...
  const updateBudget = useCallback(async (id: string, amount: number) => {
    ...
    if (error) { toast({ title: 'Error updating budget', description: error.message, variant: 'destructive' }); return false; }
  ...
  const deleteBudget = useCallback(async (id: string) => {
    ...
    if (error) { toast({ title: 'Error deleting budget', description: error.message, variant: 'destructive' }); return false; }
    setBudgets(prev => prev.filter(b => b.id !== id));
    return true;
  }, [toast]);

  return { budgets, summary, insights, isLoading, fetchBudgets, createBudget, updateBudget, deleteBudget };
}
